 ...
 * @author Brian Parra
 */
class Stsz extends \Flare\FullBox {
    ...
    public function loadData() {
        ...
        $this->readHeader();
        $this->sampleSize = \Flare\ByteUtils::readUnsingedInteger($this->file);
        if ($this->sampleSize == 0) {
            $this->sampleCount = \Flare\ByteUtils::readUnsingedInteger($this->file);
            for ($i = 0; $i < $this->sampleCount; $i++) {
                $this->sampleSizeTable[] = \Flare\ByteUtils::readUnsingedInteger($this->file);
            }
        }
    }
    ...
    public function writeToFile() {
        ...
        \Flare\ByteUtils::writeUnsignedInteger($this->file, $this->sampleSize);
        
        if ($this->sampleSize == 0) {
            \Flare\ByteUtils::writeUnsignedInteger($this->file, $this->sampleCount);
            for ($i = 0; $i < $this->sampleCount; $i++) {
                \Flare\ByteUtils::writeUnsignedInteger($this->file, $this->sampleSizeTable[$i]);
            }
        }
    }
