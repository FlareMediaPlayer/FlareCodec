 ...
 * @author Brian Parra
 */
class Elst extends \Flare\Formats\Iso\FullBox {
    ...
    function __construct($file) {
        ...
        $this->boxType = \Flare\Formats\Iso\Box::ELST;
    ...
    public function loadData() {
        ...
        for ($i = 0; $i < $this->entryCount; $i++) {
            $this->segmentTable[$i]["segmentDuration"] = \Flare\Common\ByteUtils::readUnsingedInteger($this->file);
            $this->segmentTable[$i]["mediaTime"] = \Flare\Common\ByteUtils::readUnsingedInteger($this->file);
        }
        $this->mediaRateInteger = \Flare\Common\ByteUtils::readUnsignedShort($this->file);
        $this->mediaRateFraction = \Flare\Common\ByteUtils::readUnsignedShort($this->file);
    }
    ...
    public function writeToFile() {
        ...
        for ($i = 0; $i < $this->entryCount; $i++) {
            ...
            \Flare\Common\ByteUtils::writeUnsignedInteger($this->file, $this->segmentTable[$i]["segmentDuration"]);
            \Flare\Common\ByteUtils::writeUnsignedInteger($this->file, $this->segmentTable[$i]["mediaTime"]);
        }
        
        \Flare\Common\ByteUtils::writeUnsignedShort($this->file, $this->mediaRateInteger);
        \Flare\Common\ByteUtils::writeUnsignedShort($this->file, $this->mediaRateFraction);
    }
