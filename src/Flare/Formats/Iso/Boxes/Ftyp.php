 ...
 * @author mac
 */
class Ftyp extends \Flare\Formats\Iso\Box {
    ...
    function __construct($file) {
        ...
        $this->boxType = \Flare\Formats\Iso\Box::FTYP;
    ...
    public function loadData() {
        ...
        $this->majorBrand = \Flare\Common\ByteUtils::read4Char($this->file);
        $this->minorVersion = \Flare\Common\ByteUtils::readUnsingedInteger($this->file);


        while (ftell($this->file) < ($this->size + $this->offset)) {
            $this->compatibleBrands[] = \Flare\Common\ByteUtils::read4Char($this->file);
        }
    }
    ...
    public function writeToFile() {
        ...
        \Flare\Common\ByteUtils::writeUnsignedInteger($this->file,$this->size); //Write the box size
        \Flare\Common\ByteUtils::writeChars($this->file, $this->boxType); //Write the box type
        \Flare\Common\ByteUtils::writeChars($this->file, $this->majorBrand); //Write the brand
        \Flare\Common\ByteUtils::writeUnsignedInteger($this->file, $this->minorVersion); //Write version
        for ($i = 0; $i < $compatibleBrandCount; $i++) {
            \Flare\Common\ByteUtils::writeChars($this->file, $this->compatibleBrands[$i]);
        }
