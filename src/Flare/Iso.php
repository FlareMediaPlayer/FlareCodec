...
class Iso {
    ...
    function __construct($filename) {
        ...
        $this->boxMap = [];
         //$test = new \Flare\Boxes\Mdat($this->file);
        //$this->movie = new \Flare\Presentation\Movie($this);
        //$this->movie = new Presentation\nob();
        //$this->loadData();
        //var_dump($this->boxMap);
        //$this->initMovie();
        $this->movie = new \Flare\Presentation\Movie($this); //Initialize the presentation holder
    }
    ...
    public function loadData() {
        ...
        do {
            ...
            $newBox = \Flare\Box::parseTopLevelBox($this->file, $offset, $this);
            
            if($newBox instanceof \Flare\Boxes\Moov){
                $this->moov = $newBox;
    ...
    function getFtyp(){
        foreach($this->boxMap as $box){
            if( $box instanceof \Flare\Boxes\Ftyp)
                return $box;
        }
    }
    ...
    public function getMoov(){
        foreach($this->boxMap as $box){
            if( $box instanceof \Flare\Boxes\Moov)
                return $box;
        }
    }
    ...
    public static function RipAudio($inputIso, $outputFile){
       ...
       //var_dump(get_class($inputIsovar));
        ...
        $ftyp = new \Flare\Boxes\Ftyp($iso->getFile());
        ...
        $free = new \Flare\Boxes\Free($iso->getFile());
        $iso->addBox($free);
        ...
        $mdat = new \Flare\Boxes\Mdat($iso->getFile()); //This is what will be used to control movie recording/reading
        ...
        $dataBuffer = new \Flare\DataBuffer();

        foreach ($audioTracks as $track){
            
            //$audioTrack = new \Flare\Presentation\AudioTrack($track);
    ...
    public function addMappedAudioTrack($trak){
        ...
        return \Flare\Presentation\Movie::createMappedAudioTrack($this->movie, $trak);
    }
    
    public function addNewAudioTrack(){

        return \Flare\Presentation\Movie::createNewAudioTrack($this->movie);
    }
