...
class AudioTrack extends \Flare\Presentation\Track {
    ...
    public function __construct($movie) {
        ...
        $this->trak = new \Flare\Boxes\Trak($this->file);


        $this->tkhd = new \Flare\Boxes\Tkhd($this->file);
        $this->tkhd->setVolume(1);
        $this->trak->addBox($this->tkhd);
        
        $this->edts = new \Flare\Boxes\Edts($this->file);
        $this->trak->addBox($this->edts);
        
        $this->elst = new \Flare\Boxes\Elst($this->file);
        $this->edts->addBox($this->elst);

        $this->mdia = new \Flare\Boxes\Mdia($this->file);
        $this->trak->addBox($this->mdia);

        $this->mdhd = new \Flare\Boxes\Mdhd($this->file);
        $this->mdia->addBox($this->mdhd);

        $this->hdlr = new \Flare\Boxes\Hdlr($this->file);
        $this->hdlr->setHandlerType(\Flare\Boxes\Hdlr::SOUN);
        $this->mdia->addBox($this->hdlr);

        $this->minf = new \Flare\Boxes\Minf($this->file);
        $this->mdia->addBox($this->minf);

        $this->smhd = new \Flare\Boxes\Smhd($this->file);
        $this->minf->addBox($this->smhd);

        $this->dinf = new \Flare\Boxes\Dinf($this->file);
        $this->minf->addBox($this->dinf);

        $this->dref = new \Flare\Boxes\Dref($this->file);
        $this->dinf->addBox($this->dref);

        $this->url = new \Flare\Boxes\Url($this->file);
        $this->dref->addBox($this->url);

        $this->stbl = new \Flare\Boxes\Stbl($this->file);
        $this->minf->addBox($this->stbl);

        $this->stsd = new \Flare\Boxes\Stsd($this->file);
        $this->stbl->addBox($this->stsd);

        $this->sampleEntry = new \Flare\Boxes\SampleEntries\Mp4a($this->file);
        $this->sampleEntry->setSampleRate($this->sampleRate);
        $this->sampleEntry->setDataReferenceIndex(1);
        
        $this->esds = new \Flare\Boxes\Esds($this->file);
        $this->sampleEntry->addBox($this->esds);
        
        $this->stsd->addBox($this->sampleEntry);

        $this->stts = new \Flare\Boxes\Stts($this->file);
        $this->stbl->addBox($this->stts);

        $this->stsc = new \Flare\Boxes\Stsc($this->file);
        $this->stbl->addBox($this->stsc);

        $this->stsz = new \Flare\Boxes\Stsz($this->file);
        $this->stbl->addBox($this->stsz);

        $this->stco = new \Flare\Boxes\Stco($this->file);
    ...
    private function buildDecodeTable() {
        ...
        $this->tkhd = $this->trak->getBoxByClass('\Flare\Boxes\Tkhd');
        $this->mdia = $this->trak->getBoxByClass('\Flare\Boxes\Mdia');
        $this->mdhd = $this->mdia->getBoxByClass('\Flare\Boxes\Mdhd');
        $this->hdlr = $this->mdia->getBoxByClass('\Flare\Boxes\Hdlr');
        $this->minf = $this->mdia->getBoxByClass('\Flare\Boxes\Minf');
        $this->stbl = $this->minf->getBoxByClass('\Flare\Boxes\Stbl');


        $this->stts = $this->stbl->getBoxByClass('\Flare\Boxes\Stts');
        $this->stsc = $this->stbl->getBoxByClass('\Flare\Boxes\Stsc');
        $this->stsz = $this->stbl->getBoxByClass('\Flare\Boxes\Stsz');
        $this->stco = $this->stbl->getBoxByClass('\Flare\Boxes\Stco');
